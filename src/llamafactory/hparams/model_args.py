...
class ModelArguments(QuantizationArguments, ProcessorArguments, ExportArguments, VllmArguments):
    ...
    zero_mask: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to use important neurons for fine-tuning."},
    )

    fine_loss: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to use an improved loss function for training"},
    )

    finetune_class: Optional[str] = field(
        default="gsm",
        metadata={"help": "What parameter matrix is used for model fine-tuning"},
    )
