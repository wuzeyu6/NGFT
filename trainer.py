...
class Trainer:
    ...
    def __init__(
        ...
        self.fintune_class = fintune_class
        self.args = args
            ...
            for _ in range(total_updates):
                ...
                for inputs in batch_samples:
                    ...
                    with self.accelerator.accumulate(model):
                        tr_loss_step, entropy_loss = self.training_step(model, inputs, num_items_in_batch, batch_size=batch_size, batch_num=batch_num, epoch_step = epoch, epoch=batch_num/total_updates, bround=bround, loss_temp=loss_temp, min_loss=min_loss, loss_open=loss_open)
                    ...
                    if (
                        ...
                        loss_save = loss_save + entropy_loss
                    ...
                    if do_sync_step:
                        ...
                        # list_average.append(np.mean(list_loss))
                        ...
                        if self.zero_mask == 1:
                            ...
                            if hasattr(model, "module"):
                                layer = len(model.module.model.layers)
                            else:
                                layer = len(model.model.layers)
                            with open('neuron_weight/{}.pkl'.format(self.fintune_class), 'rb') as file:
                                important_neuron = pickle.load(file)

                            for name, param in model.named_parameters():
                                if param.grad == None:
                                    continue
                                if 'up proj' not in name and 'down_proj' not in name and 'gate_proj' not in name:
                                    continue
                                if 'mlp' in name and 'gate_proj' in name:
                                    for i in range(layer):
                                        if str(i) in name:
                                            mask = torch.zeros_like(param.grad)
                                            # temp = important_neuron[1][i].copy()
                                            # temp_1 = [x - 17 for x in temp]
                                            # mask[temp_1, :] = 1
                                            mask[important_neuron[0][i], :] = 1
                                            param.grad *= mask
                                    ...
                                    for j in range(layer):
                                        if str(j) in name:
                                            mask = torch.zeros_like(param.grad)
                                            # temp = important_neuron[0][i].copy()
                                            # temp_1 = [x - 1 for x in temp]
                                            # mask[temp_1, :] = 1
                                            mask[important_neuron[1][j], :] = 1
                                            param.grad *= mask
                                    ...
                                    for m in range(layer):
                                        if str(m) in name:
                                            mask = torch.zeros_like(param.grad)
                                            # temp = important_neuron[2][i].copy()
                                            # temp_1 = [x - 7 for x in temp]
                                            # mask[temp_1, :] = 1
                                            mask[important_neuron[2][m], :] = 1
                                            param.grad *= mask
    ...
    def training_step(
        ...
        batch_size=None, batch_num=None, epoch_step=None, epoch=None, bround=0, loss_temp=None, list_loss=[], min_loss=None, loss_open=False) -> torch.Tensor:
        """
        ...
        def find_max_drop_token_index(model, inputs, tokenizer):
            ...
            for i, step_logits in enumerate(scores):
                ...
                token_ppl = 1.0 / (token_prob + 1e-9)
                ppl_values.append(token_ppl)
            ...
            for i in range(1, search_end):
                current_ppl = ppl_values[i]
                next_ppl = ppl_values[i + 1]

                if next_ppl > 1e-9:
                    ratio = current_ppl / next_ppl
                else:
                    ratio = 0.0
                ...
                if match:
                    ...
                    if matched_data:
                        messages.append([
                            ...
                            {"role": "user", "content": matched_data["input"]},
                            {"role": "assistant", "content": matched_data["output"]}
                        ])
                        found_match = True
        ...
        if self.fine_loss != 1:
            ...
            if epoch_step >= 3:
                ...
                if "Llama" in self.fintune_class:
                    model_path_ = "meta-llama/Meta-Llama-3-8B-Instruct"
                elif "Qwen2.5-7B" in self.fintune_class:
                    model_path_ = "Qwen/Qwen2.5-7B-Instruct"
                elif "Qwen2.5-0.5B" in self.fintune_class:
                    model_path_ = "/mnt/shared-storage-user/large-model-center-share-weights/hf_hub/models--Qwen--Qwen2.5-0.5B-Instruct/snapshots/7ae557604adf67be50417f59c2c2f167def9a775"
                elif "Qwen2.5-1.5B" in self.fintune_class:
                    model_path_ = "Qwen/Qwen2.5-1.5B-Instruct"
                elif "Qwen2.5-3B" in self.fintune_class:
                    model_path_ = "Qwen/Qwen2.5-3B-Instruct"
                ...
                # now_inputs, token_index, max_length = gengerate_best_inputs(tokenizer, inputs = inputs, data = data, batch_size=batch_size, batch_num=batch_num,
                #                                    now_output=1, model=model)
                # now_inputs = now_inputs.to(model.device)
                #
                # best_inputs = gengerate_best_inputs(tokenizer, inputs = inputs, data = data)
                # best_inputs = best_inputs.to(model.device)
                gate, up, down = self.compute_loss_2(model, inputs, tokenizer=tokenizer)
                from safetensors.torch import load_file
                loaded_data = load_file("BioInstruct_Qwen2.5-0.5-Instruct.safetensors")
                avg_gate = loaded_data["avg_gate"].to(model.device)
                avg_up = loaded_data["avg_up"].to(model.device)
                avg_down = loaded_data["avg_down"].to(model.device)
                # gate_, up_, down_ = self.compute_loss_1(model, inputs, data, tokenizer=tokenizer)

                from neruon_simliar import overlapping_cosine_similarity, kl_similarity
                ...
                #     return torch.norm(torch.cat(valid_grads), p=2)
                #
                # norm_1 = compute_grad_norm(grads_1)
                # norm_2 = compute_grad_norm(grads_2)
            ...
            # if loss_entropy < 0.1:
            #     loss_sim = loss_sim_mse
            #     params = [p for p in model.parameters() if p.requires_grad]
            #     grad_entropy_list = torch.autograd.grad(loss_entropy, params, retain_graph=True, allow_unused=True)
            #     grad_entropy_list = [g for g in grad_entropy_list if g is not None]
            #
            #     grad_sim_list = torch.autograd.grad(loss_sim, params, retain_graph=True, allow_unused=True)
            #     grad_sim_list = [g for g in grad_sim_list if g is not None]
            #
            #     if grad_entropy_list and grad_sim_list:
            #         grad_entropy_norm = torch.cat([g.flatten() for g in grad_entropy_list]).norm()
            #         grad_sim_norm = torch.cat([g.flatten() for g in grad_sim_list]).norm()
            #
            #         epsilon = 1e-8
            #         if grad_entropy_norm > epsilon and grad_sim_norm > epsilon:
            #             print("grad_entropy_norm:", grad_entropy_norm)
            #             print("grad_sim_norm:", grad_sim_norm)
            #             total_norm = grad_entropy_norm + grad_sim_norm
            #             w_entropy = grad_sim_norm / total_norm
            #             w_sim = grad_entropy_norm / total_norm
            #             loss = w_entropy * loss_entropy + w_sim * loss_sim

            else:
                loss = 1.0 * loss_entropy
        ...
        if self.fine_loss == 1:
            return loss.detach() / self.args.gradient_accumulation_steps, loss_entropy.item()
        else:
            return loss.detach() / self.args.gradient_accumulation_steps, 0.0
    ...
    def compute_loss_2(self, model, inputs, generate_tokens=50, tokenizer=None):
        ...
        class LlamaFeatureExtractor:
            ...
            def register_hooks(self):
                ...
                def up_hook(module, input, output):
                    self.up_outputs.append(torch.mean(output[:, :, :], dim=1))
        ...
        try:
            ...
            # [Layers, Dim]
            final_gate = torch.mean(last_token_gate, dim=1)
            final_up = torch.mean(last_token_up, dim=1)
            final_down = torch.mean(last_token_down, dim=1)

        finally:
            extractor.cleanup()
            del extractor

        return final_gate, final_up, final_down
    ...
    def compute_loss_1(self, model, inputs, data, tokenizer=None):
        ...
        class LlamaFeatureExtractor:
            ...
            def register_hooks(self):
                ...
                def up_hook(module, input, output):
                    self.up_outputs.append(torch.mean(output[:, self.prompt_len:, :], dim=1))
        ...
        for idx in range(len(input_ids)):
            ...
            answer_tokens = \
            tokenizer(target_output_str, add_special_tokens=False, return_tensors="pt")['input_ids'][0].to(
                input_ids.device)
            combined = torch.cat([curr_input_ids, answer_tokens], dim=0)
            full_input_ids_list.append(combined)
        ...
        try:
            ...
            # final_down = torch.mean(torch.mean(target_down, dim=2), dim=1)
            final_gate = torch.mean(target_gate, dim=1)
            final_up = torch.mean(target_up, dim=1)
            final_down = torch.mean(target_down, dim=1)
    ...
    #
    #             def up_hook(module, input, output):
    #                 self.up_outputs.append(output.unsqueeze(0))
    ...
    #                 down_hook_handle = layer.mlp.down_proj.register_forward_hook(down_hook)
    ...
    #         gate_ = gate_[:,:,max_length:,:]
    #         up_ = up_[:,:,max_length:,:]
    #         down_ = down_[:, :,max_length:,:]
    #
    #         gate_ = torch.mean(gate_,dim=2)
    #         up_ = torch.mean(up_, dim=2)
    #         down_ = torch.mean(down_, dim=2)
    #
    #         gate_ = torch.mean(gate_, dim=1)
    #         up_ = torch.mean(up_, dim=1)
    #         down_ = torch.mean(down_, dim=1)
    #
    #
    #     finally:
    #         # 强制清理提取器（无论是否报错都执行）
    ...
    # def compute_loss_1(self, model, inputs, return_outputs=False, num_items_in_batch=None):
    #     """
    #     How the loss is computed by Trainer. By default, all models return the loss in the first element.
    #
    #     Subclass and override for custom behavior.
